# Hi, I'm Hoa Minh Khoi 👋

Final-year Finance student interested in **Quantitative Research**, **Trading Analytics**, and **Financial Data Science**.

## Areas of Interest

- Quantitative Portfolio Management
- Market Risk Modeling
- Algorithmic Trading
- Time Series Analysis
- Financial Econometrics

## Technical Skills

**Programming**

Python • R • SQL

**Libraries**

NumPy • Pandas • SciPy • Matplotlib • Statsmodels • Scikit-learn

**Finance**

Portfolio Optimization • VaR • Expected Shortfall • Monte Carlo Simulation • Backtesting • CAPM • Fama-French

## Selected Projects

- Entropy-Based Portfolio Optimization with Varma–Tsallis Generalized Growth
- Portfolio Risk Assessment using VaR, Expected Shortfall and Statistical Backtesting
- Monte Carlo Simulation for Investment Risk Analysis

## Contact

- LinkedIn: *(thêm link của bạn)*
- Email: *(email của bạn)*## Hi there 👋

<!--
**HMK05/HMK05** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
